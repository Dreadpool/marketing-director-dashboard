...
- [x] Meta Ads Dashboard metric tooltips (2026-05-15): Hover-tooltip definitions for every metric abbreviation in the Meta Ads workflow visualization (CVR, CTR, CPA, CPM, ROAS, Frequency, Hook Rate, Hold Rate). Shared `MetricLabel` component + `METRIC_DEFINITIONS` map at `src/components/workflows/metric-label.tsx` (single source of truth, typed `MetricName` union). Wired into `meta-ads-fetch-summary.tsx` (campaign hierarchy headers, inline CVR/CTR labels in ad rows, Highest CPA Ads / HiringTable / BreakdownTable headers) and `steps/cpa-diagnostic.tsx` (D1–D5 substep headers + D5 summary stack labels). Existing `MetricCard` KPI-card tooltips (CPA/ROAS with SLE-specific over-attribution copy) left untouched. Generalizes the "Tooltips for hook rate and hold rate" Up Next item.
