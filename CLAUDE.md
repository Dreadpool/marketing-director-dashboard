...
- [x] Ad Health Classification System (2026-04-09; profit thresholds retired 2026-07-18): per-ad and per-ad-set diagnostics use engagement, relative campaign performance, and change over time. CPA and ROAS remain reported signals but do not produce profitability or spend verdicts until route economics are rerun.
...
- [ ] Google Ads Analysis workflow: preserve brand/non-brand segmentation, search-quality diagnostics, ground-truth comparison, and trends. Do not restore CPA/ROAS profit thresholds until route economics and incrementality are available.
...
- [ ] Meta Ads Guided Evaluation Phase 2: Add Steps 2-5 (Backend Verification, Campaign Structure, Creative Health, Audience Check) to the guided evaluation. Step 2 needs BigQuery cross-reference (Meta purchases vs actual bookings, blended CAC, MER). Steps 3-5 use existing fetched data with new evaluation logic. Design spec at `docs/specs/2026-03-30-meta-ads-guided-evaluation-design.md`.
...
- [ ] Cohort repeat-value tracking: group purchasing emails by first real booking and compare cumulative booking value and repeat rate at 12/24/36 months. Do not convert the result into a profit or CPA ceiling until current route costs and incrementality are available.
...
- 2026-03-30: Period-matched scheduling replaces timestamp-based isDue. A run satisfies the period it was run FOR, not when it was run. Per-workflow due dates (1st, 3rd, 10th, 1st Monday). Cadence config in code, not DB. Full spec in `docs/specs/2026-03-30-workflow-cadence-scheduling-design.md`.
